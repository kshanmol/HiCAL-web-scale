...
class GetMarginalPrecisionAJAXView(views.CsrfExemptMixin, views.LoginRequiredMixin, views.JsonRequestResponseMixin, generic.View):
    require_json = False

    def get(self, request, window_size, *args, **kwargs):
        try:
        ...
        current_session_id = str(self.request.user.current_task.uuid)
        if 'max_marginal_precision' not in request.session:
            request.session['max_marginal_precision'] = {}
            request.session.modified = True
        
        if current_session_id not in request.session['max_marginal_precision'].keys():
            request.session['max_marginal_precision'][current_session_id] = 0 # This was not staying persistent, needed the session.modified bool to be flipped below
            request.session.modified = True
                
        result = {}
        window_rel_count, total_rel_count = 0, 0
        for judgment in latest:
            if judgment.relevance > 0:
                total_rel_count += 1
        for judgment in latest[:window_size]:
            if judgment.relevance > 0:
                window_rel_count += 1
        marg_precision = window_rel_count/len(latest[:window_size])
        total_precision = total_rel_count/len(latest)
        if marg_precision > request.session['max_marginal_precision'][current_session_id]:
            request.session['max_marginal_precision'][current_session_id] = marg_precision
            request.session.modified = True
        result['mp'] = marg_precision
        result['tp'] = total_precision
        result['max_mp'] = request.session['max_marginal_precision'][current_session_id]
        result['num_judgments'] = len(latest[:window_size])
        result['total_judgments'] = len(latest)
        return self.render_json_response(result)
